...
export async function captureAllScreenshots(
  ...
  try {
    ...
    config.renderer.setSize(targetSize, targetSize, false);
...
export function updateDownloadButton(downloadBtn: HTMLButtonElement, datasetCount: number) {
  ...
  if (dpiSelect) {
    dpiSelect.disabled = isDisabled;
    if (isDisabled) {
      dpiSelect.title = 'Upload a dataset to enable downloads';
    } else {
      dpiSelect.removeAttribute('title');
    }
  }
}
